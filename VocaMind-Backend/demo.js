...
// Topical Guardrail Setup (remains the same as previous code)
...
async function makeOutboundCall(phoneNumber, language = 'en', name = '') {
  try {
    if (activeCallCount >= MAX_CONCURRENT_CALLS) {
      ...
      callQueue.push({ phoneNumber, language, name });
      return;
    }
    
    activeCallCount++;
    console.log(`Making outbound call to ${phoneNumber} (${name}) in ${language}. Active calls: ${activeCallCount}`);
              ...
              <Response>
                <Say language="${
                  language === 'es'
                    ...
                    ? 'ko-KR'
                    : 'en-US'
                }">Hello, ${name || 'there'}. Welcome to WorkOnward Resume Assistant. We will begin our conversation shortly.</Say>
                <Connect>
                  <Stream url="${SERVER_URL.replace('http', 'ws')}/media-stream">
...
function loadPhoneNumbersFromCSV(filePath) {
  try {
    ...
    for (const record of records) {
      ...
      const nameField = record.name || record.Name || record.NAME || '';
...
fastify.post("/batch-calls", async (request, reply) => {
  try {
    const filePath = request.body?.filePath || path.join(__dirname, 'phone_numbers.csv');
    ...
    const callPromises = initialBatch.map(entry => 
      makeOutboundCall(entry.phoneNumber, entry.language, entry.name)
    );
...
fastify.listen({ port: PORT, host: '0.0.0.0' }, (err) => {
  ...
  console.log(`To make outbound calls, POST to ${SERVER_URL}/batch-calls`);
